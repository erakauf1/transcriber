...
export function buildSystemPrompt(language) {
  ...
  return [
    ...
    `- Add paragraph breaks between distinct topics or thoughts`,
    `- When the speaker lists items, steps, tasks, or options, format them as a bulleted list using "• " (bullet + space) at the start of each item, one item per line`,
    `- Use line breaks to separate logical sections — don’t cram everything into one block of text`,
    ``,
    `Don't:`,
